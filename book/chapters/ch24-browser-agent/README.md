...
本章从理论铺垫开始：Browser Agent 的四大挑战（DOM 无穷大、页面跳变、反爬、登录态），以及这四个挑战如何精确映射到你在前 23 章建立的六大支柱上。然后我们看三条浏览器路径的决策逻辑，深入 browser-use（93k+ stars）的架构，组装出 `lena-v0.24`，跑通三个端到端任务。
...
**数字化的差距**（以下数字基于作者实战观察，非标准 benchmark 数据，实际结果因网站和任务而异）：
...
**论文引用**：WebArena（2023，Zhou et al.）是浏览器 agent 的标准测评集，包含 812 个真实 web 任务。在实战观察中，页面跳变错误（agent 在跳转后仍尝试操作上一页面的元素）是最常见的失败原因之一。不需要读完这篇论文，只需要知道：**页面跳变处理能力是 browser agent 成熟度的重要指标**。
