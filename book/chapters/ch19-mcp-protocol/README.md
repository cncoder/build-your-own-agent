...
Simon Willison 在 2025 年 10 月 16 日称 Skills "在某种意义上比 MCP 影响更大"（来源：R3-ecosystem-2025.md §二.2）。他的逻辑：MCP 让 agent 能连接更多工具，而 Skills 改变了 agent 获取方法论的方式——任何人都能把最佳实践封装成 Skill 分发给所有 agent，这比工具更底层。随后 OpenAI 在 2025 年 12 月也悄悄在 ChatGPT 和 Codex CLI 中加入了 Skills。
...
这个观察与 R17 对 33 条大厂 JD 的分析高度吻合：**33% 的 AI Agent 工程师岗位明确要求多 agent 协作经验**，且岗位描述中频繁出现"agent orchestration"、"inter-agent communication"、"async task delegation"这些词。
...
1. MCP 的三个字母分别代表什么，核心价值是什么？
2. MCP 和 REST API 的根本区别在哪里？
3. stdio 传输模式下，三个通道（stdin/stdout/stderr）各负责什么？
4. 为什么 MCP 优先选择 stdio 而不是 HTTP 传输？
5. JSON-RPC 2.0 的三种消息类型是什么，如何区分？
6. `tools/list` 请求返回什么，格式是什么？
7. `initialize` 握手的两步是什么，缺一步会怎样？
8. `notifications/initialized` 为什么没有 `id` 字段？
9. MCPClient 里 `_pending` 字典的 key 和 value 分别是什么？
10. `asyncio.Future` 在 MCP 实现里解决了什么问题？
11. `_read_loop` 在 EOF 时应该做什么，不做会怎样？
12. PIPE_BUF 是什么，通常大小是多少？
13. 不排空 stderr 时，死锁的完整路径是什么（6 步）？
14. 为什么 stderr 死锁难以排查？
15. `_stderr_loop` 和 `_read_loop` 必须并发运行，为什么？
16. FastMCP 的市场占有率是多少，现在在哪里安装？
17. MCP 工具名 `filesystem__read_file` 的 `__` 双下划线是为了解决什么问题？
18. MCP 和 Skills 各自适合什么场景？
19. MCP 的两个主要安全风险分别是什么？
20. 给 filesystem MCP server 只传 `/tmp` 而不是 `/` 体现了什么原则？
...
- R3 生态报告：`docs/research/R3-ecosystem-2025.md`（FastMCP 100 万日下载数据来源）
