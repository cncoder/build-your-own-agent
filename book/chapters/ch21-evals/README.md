...
但这个 >80% 的数字需要在正确的条件下才能复现。Hamel Husain 在 hamel.dev/blog/posts/llm-judge/ 提出的 **Critique Shadowing 7 步法**给出了提升人机一致性的工程路径——核心是用 pass/fail + 详细 critique 替代数字评分：
...
- 作者对公开 AI Agent 岗位描述的非正式调研：超过半数要求 eval 能力，task completion rate + tool selection accuracy 是最高频的两个 agent-specific 指标
