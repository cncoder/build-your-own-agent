...
基于对 56 份 AI Agent 岗位描述的分析，超过半数明确要求 eval 能力，工业界最看重的两个 agent-specific 指标是 **task completion rate** 和 **tool selection accuracy**——这两个都在上面这张表里。
...
- 基于对 56 份 AI Agent 岗位描述的分析：超过半数要求 eval 能力，task completion rate + tool selection accuracy 是最高频的两个 agent-specific 指标
