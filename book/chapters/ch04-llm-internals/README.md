...
你打开 Anthropic 文档，看到 Claude Opus 4、Sonnet 4.5、Haiku 3.5 并排排列。你决定用 Opus，因为"更聪明"。
...
Anthropic 白皮书在架构选型章节给出了一个工程师级的选型公理：
...
> （来源：Anthropic, *Building Effective AI Agents*, 2025, p.10）
...
- 成本优先 → DeepSeek-V3（中文）或 Claude Haiku（英文/工具）
...
换 Haiku：输入 $0.8/M，输出 $4/M → $5.6 + $4 = $9.6/天 = $288/月

如果质量可接受，Haiku 节省 73%。这个计算应该在写第一行 agent 代码之前做。
...
Haiku:  {'daily_usd':  5.7, 'monthly_usd': 171.0, 'cache_saving_monthly_usd': 117.6}
```

这个函数是 Beat 5 "信封背面计算"的代码版本。在设计 agent 之前跑一次，把感受从"大概多少钱"变成"确切多少钱"。缓存命中率 70% 时，Sonnet 的月成本从 $1,080 降到 $639，节省 $441——比换模型更简单，且没有质量损失。
