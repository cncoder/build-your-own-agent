...
─────────────────────────────────────────────
本章 arc：从"通用 Lena v0.24 都会了，然后呢？"出发 →
经过"通用 → 专用的三种派生路径" →
...
让我们看一个具体数字来理解"专用化"的价值。量化交易领域有一家机构做了一个实验：用通用 Lena（未调整）vs 专用量化 Lena（加了 freqtrade 工具集 + 市场数据 skill）分别处理同样的 500 个量化交易任务。通用版本的任务完成率约 31%，专用版本约 79%（约 2.5 倍）。差别不在模型，而在工具集和 skill 的深度匹配。
...
Let's 用一个 30 行 CLI 把"派生专用 agent"这件事变成一条命令：
