...
【窦文涛】Anthropic 在 Scaling Managed Agents（2026-04-08）里用 Pets vs Cattle 这个运维隐喻总结了为什么这样设计：宠物是有名字、照料的个体，死了不可替代；牛群是无状态、可替换的个体，挂了自动重生。没有 Heartbeat 的 Lena 是宠物——进程死了你才发现她不在跑。有了 Heartbeat 和独立告警通道，Lena 变成牛群——外部调度器保证她活着，告警通道保证你知道她死了。
