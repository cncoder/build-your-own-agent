...
Anthropic 的 Managed Agents 设计中引用了 ops 经典隐喻 **Pets vs Cattle**：
...
| # | 检查项 | 结果 |
|---|--------|------|
| S1 | Beat 1 有"从 A 出发经过 B 到 C，途中踩坑 D"的路线图叙事 | YES |
| S2 | Beat 2 有具体数字（24 小时 0 条消息，以及 6.5 小时无通知的具体场景）| YES |
| S3 | Beat 3 有 ≥2 个标注"纯理论"的小节 | YES（3.1 / 3.2 / 3.3 三节）|
| S4 | Beat 4 有 Let's 句型，代码 ≤80 行 | YES |
| S5 | Beat 5 有扩展表格，每步有中间验证输出 | YES |
| S6 | Beat 6 有具体预期输出（timestamp + Telegram 消息内容）| YES |
| S7 | Beat 6 最后一句是指向下一章的叙事钩子 | YES |
| S8 | Beat 7 标题是疑问句 | YES |
| C1 | 核心技术主张有来源（Karpathy 原话 + openclaw 公开仓库 file 引用）| YES |
| C2 | Convention 消歧句覆盖所有核心术语对（Reactive/Proactive, 主通道/独立告警通道）| YES |
| C3 | 有失败路径声明（Telegram 401 + outside active hours 诊断）| YES |
| C4 | 无独立 case 块，无超过 1 个显式案例 | YES |
| C5 | 有明确立场（178 行 vs 4 子模块的选型建议，有具体条件）| YES |
| R1 | 无"案例 X.Y"格式条目块 | YES |
| R2 | 无本机绝对路径 | YES |
| R3 | 无作者真实姓名 | YES |
| R4 | 无"为了更好地"虚假动机 | YES |
| R5 | 无无立场的"都可以"段落 | YES |
| R6 | 无先给完整代码再解释的 Fine-to-Coarse 反模式 | YES |
| R7 | 所有新术语（Reactive/Proactive/active-hours/独立告警通道等）首次使用前已定义 | YES |
