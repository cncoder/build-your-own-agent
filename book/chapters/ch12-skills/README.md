...
真实系统里，Claude Code 的内置工具里，`FileReadTool` 的 schema 只有 150 行，而有经验的 CC 用户在 `~/.claude/skills/` 里放了 30 个 skill 文件，每个都是 "怎么做这类任务" 的 SOP。按需加载，用的时候才占 context。
...
## Anthropic 白皮书：Skills 作为可组合架构

Anthropic 架构白皮书从架构层面定义了 Skills 的核心属性：**可组合性（composability）**。

> "Skills can work together on complex tasks and invoke other skills as needed. A compliance skill might call a document analysis skill, which in turn uses a specialized extraction skill."（来源：Anthropic, *Building Effective AI Agents: Architecture Patterns and Implementation Frameworks*, 2025, p.11）
...
白皮书把这种设计称为"能力金字塔"（capability pyramid）。本章实现的三层披露机制（元数据 → 完整 SOP → 子文件）正是这个金字塔在单个 Skill 内部的体现；而 Skill 之间的互相调用，则是金字塔在跨 Skill 层面的扩展。
...
2025-12 月，OpenAI 在 ChatGPT 和 Codex CLI 里加入了格式上高度相似的 Skills 功能。这不是竞争式跟随，而是同一个工程问题（知识复用和 context 效率）在不同产品里收敛到了相似答案。
...
```
~/.claude/skills/          ← 全局级（userSettings）
.claude/skills/            ← 项目级（projectSettings）
/managed/.claude/skills/   ← 组织策略级（policySettings）
```
