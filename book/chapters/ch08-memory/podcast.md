...
【窦文涛】区分标准很简单——这条信息是 agent 经历产生的（用户偏好、工作结论），还是用户提供的外部文档？前者用本章文件系统，后者才用向量库。两个系统的接口不同、读写时机不同、失效模式也不同，不能互换。
...
【窦文涛】Recitation 的代价：system prompt 增大约一倍，两条偏好约 50 tokens，双写约 100 tokens，可接受。记忆量很大时，末尾只写一行摘要。Recitation 是结构性解法——不依赖模型的内部注意力分布，也不需要改训练，只改上下文排列方式。

【窦文涛】运行演示：第一次启动，输入"我叫 Demo-User，我偏好 Python"，Lena 调用 `save_memory` 写两条记忆：`user_name` 和 `programming_language`。第二次新进程，`[Memories loaded: 2]`，输入"帮我写 hello world"，Lena 直接给 Python，不再反问。无记忆版本同样的输入得到"你想用哪种语言"。差距来自 1 条工具调用加 2 个 `.md` 文件。整个系统磁盘占用 < 10KB，system prompt 注入 < 500 tokens。
...
【窦文涛】五条关键设计决策：①**verbatim 优先**——session 历史存 SQLite 原文，绝不依赖 LLM 摘要，防 compaction 篡改。②**文件系统 > 向量库**——1000 条内个人记忆全量读取 50ms，向量库是过度设计。③**Recitation 双写**——system prompt 首尾各放一次长期记忆，对抗注意力衰减。④**save_memory 边界**——偏好和事实写入，临时状态不写入。⑤**max_chars=2000**——单条记忆截断保护，防 context 污染。

【窦文涛】动手两步：**已有 agent 项目**，加 `MemoryStore` 和 `MemDir`，注册 `save_memory` 工具，重启进程验证 `[Memories loaded: N]` > 0；**新项目**，先让两个类各自可运行，`sqlite3 ~/.lena/memory.db ".tables"` 看到两张表、`MemDir.save()` 产生 `.md` 文件，再接入 agent 主循环。
